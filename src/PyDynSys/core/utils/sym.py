...
from ..support.vector_field import AutVectorField, NonAutVectorField
...
class SymbolicToVectorFieldResult:
    ...
    Fields:
        vector_field: AutonomousVectorField or NonAutonomousVectorField instance
...
class SymbolicSystemBuilder:
    ...
    def build_vector_field(
        ...
        # Build appropriate vector field (as callable first)
        if is_autonomous:
            callable_field = SymbolicSystemBuilder._build_autonomous_field(
                derivatives_substituted, state_symbols, dimension
            )
            # Wrap in AutonomousVectorField
            ...
            # Wrap in NonAutVectorField
            vector_field = NonAutVectorField(
                dimension=dimension,
                callable_field=callable_field,
                symbolic_expr=derivatives_substituted
            )
