...
class TimeHorizon:
    ...
    Factories:
    - Unbounded (real_line): T = R (default)
    - Closed Interval (closed_interval(t_min, t_max)): T = [a, b]
    - Open Interval (open_interval(t_min, t_max)): T = (a, b) 
    ...
    def contains_times(self, t_arr: NDArray[np.float64]) -> bool:
        """
        Check if all times in t_arr are in T.
        
        Args:
            t_arr (NDArray[np.float64]): Times to test
            
        Returns:
            bool: True if all times in t_arr are in T, False otherwise
        """
        return np.all([self.contains_time(t) for t in t_arr])
