...
class PhaseSpace:
    ...
    def distance(
        ...
        """
        Euclidean distance between two points in phase space.
        
        Computes L2 norm: ||x1 - x2||_2 = sqrt(Σ(x1_i - x2_i)²)
        
        This is a property of the phase space metric, not the dynamical system.
        
        Args:
            x1, x2: Points in phase space R^n (must be in X)
            
        Returns:
            float: Euclidean distance
            
        Raises:
            ValueError: If x1 or x2 have incorrect dimension or are not in X
        """
        if x1.shape != (self.dimension,):
            raise ValueError(
                f"x1 has incorrect dimension: expected ({self.dimension},), got {x1.shape}"
            )
        if x2.shape != (self.dimension,):
            raise ValueError(
                f"x2 has incorrect dimension: expected ({self.dimension},), got {x2.shape}"
            )
        
        # Validate membership
        if not self.contains_point(x1):
            raise ValueError(f"x1 {x1} is not in phase space X")
        if not self.contains_point(x2):
            raise ValueError(f"x2 {x2} is not in phase space X")
        
        return float(np.linalg.norm(x1 - x2))
    
    
    def volume_of_points(self, points: NDArray[np.float64]) -> np.float64:
        """
        ...
        This is a geometric property of the point set in the phase space.
        
        Args:
            points: Array of shape (n_points, dimension) representing points in X
        ...
        Raises:
            ValueError: If points.shape[1] != dimension or points not in X
        """
        if points.ndim != 2 or points.shape[1] != self.dimension:
            raise ValueError(
                f"Points must have shape (n_points, {self.dimension}), "
                f"got shape {points.shape}"
            )
        
        # Validate all points are in phase space
        if not self.contains_points(points):
            raise ValueError("Not all points are in phase space X")
